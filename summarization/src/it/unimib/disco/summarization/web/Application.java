...
public class Application extends AbstractHandler{
	...
	@Override
	public void handle(String path, Request base, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException { //path=target of the request
		...
		try{
			Routing routes = new Routing();  //routes è l'oggetto che conteine le mappature tra rotta(string) e response
			//mi faccio la mappatura
			serviceAPI(routes);
			mainUI(routes);
			API(routes);
			experiment(routes);
			...
			routes.routeTo(path).sendTo(base, response, new HttpParameters(request)); //recupera il response giusto per il path e poi lo spedisce(sendTo)
